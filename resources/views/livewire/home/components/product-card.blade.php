    <div class="border rounded-3xl relative group {{ $viewType == 'row' ? 'lg:flex gap-4' : '' }}">
        <img src="{{ $image }}"
            class="rounded-2xl {{ $viewType == 'row' ? 'w-1/4' : 'w-full' }} aspect-video object-cover"
            alt="{{ $title }}" loading="lazy">
        <div class="p-4 w-full">
            <div class="{{ $viewType != 'row' ? 'flex w-full' : '' }}">
                <div>
                    ...
                    <div class="mt-1 inline-flex items-center gap-2">
                        <svg class="size-4 fill-yellow-500" xmlns="http://www.w3.org/2000/svg" width="32"
                ...
                <div class="flex-1 {{ $viewType == 'grid' ? 'text-right' : 'text-left' }}">
                    <p class="shrink-0 {{ $viewType == 'row' ? 'mt-2' : '' }}"><span
                            class="sm:text-lg text-neutral-800 text-right">Rp.
                            {{ number_format($price, 0, ',', '.') }}</span></p>

                    <div class="flex items-center justify-end gap-2 mt-4">
                        <button wire:click='addToFavorite("{{ encrypt($produkId) }}")'
                        ...
                        <button wire:click='addToCart("{{ encrypt($produkId) }}")'

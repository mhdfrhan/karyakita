<div>
    ...
    @if ($jasa->servicePay && $jasa->servicePay->first())
        @if ($jasa->servicePay->first()->status == 'pending' || $jasa->servicePay->first()->expiry_date < now())
            <div class="mb-6 flex items-center justify-between gap-3 flex-wrap">
                <div>
                    <span class="text-sm text-neutral-500">Note</span>
                    @if ($jasa->servicePay->first()->status == 'pending')
                        <p class="text-red-600 font-medium">Silahkan selesaikan pembayaran pada jasa kamu</p>
                    @else
                        <p class="text-red-600 font-medium">Masa aktif jasa kamu telah habis, silahkan lakukan
                            perpanjangan</p>
                    @endif
                </div>
                <x-primary-button x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-payment')">
                    {{ $jasa->servicePay->first()->status == 'pending' ? 'Bayar sekarang' : 'Perpanjang sekarang' }}
                </x-primary-button>
            </div>
        @endif
    @else
        <div class="mb-6 flex items-center justify-between gap-3 flex-wrap">
            <div>
                <span class="text-sm text-neutral-500">Note</span>
                <p class="text-red-600 font-medium">Silahkan lakukan pembayaran pada jasa kamu</p>
            </div>
            <x-primary-button x-data=""
                x-on:click.prevent="$dispatch('open-modal', 'confirm-payment')">Bayar sekarang</x-primary-button>
        </div>
    @endif

    @if (
        !$jasa->servicePay ||
            !$jasa->servicePay->first() ||
            $jasa->servicePay->first()->status == 'pending' ||
            $jasa->servicePay->first()->expiry_date < now())
        <x-modal align="center" maxWidth="5xl" name="confirm-payment" :show="$errors->isNotEmpty()" focusable>
            <form wire:submit.prevent="submitPayment" class="p-6">

                <h2 class="text-lg font-medium text-neutral-900">
                    {{ __('Lakukan Pembayaran Pada Jasa Kamu') }}
                </h2>

                <p class="mt-1 text-sm text-neutral-600">
                    {{ __('Harga setiap iklan jasa bervariasi, mulai dari Rp. 10.000/bulan. Silahkan pilih paket pembayaran dibawah ini') }}
                </p>
                    ...
                    <div class="w-1/3 px-2">
                        <label
                            ...
                            <input type="radio" id="1month" class="sr-only peer" name="service-price"
                                value="1" wire:model.live='servicePrice'>
                            <h3 class="text-lg font-semibold text-center mb-4">Paket 1 Bulan</h3>
                            <div>
                                <h1 class="text-center text-2xl font-semibold">Rp. 10.000</h1>
                            </div>
                        </label>
                    </div>
                    <div class="w-1/3 px-2">
                        <label
                            class="p-4 border rounded-2xl has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-500 has-[:checked]:text-white duration-150 block cursor-pointer">
                            <input type="radio" id="3month" class="sr-only peer" name="service-price"
                                value="3" wire:model.live='servicePrice'>
                            <h3 class="text-lg font-semibold text-center mb-4">Paket 3 Bulan</h3>
                            <div>
                                <h1 class="text-center text-2xl font-semibold">Rp. 25.000</h1>
                            </div>
                        </label>
                    </div>
                    <div class="w-1/3 px-2">
                        <label
                            class="p-4 border rounded-2xl has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-500 has-[:checked]:text-white duration-150 block cursor-pointer">
                            <input type="radio" id="6month" class="sr-only peer" name="service-price"
                                value="6" wire:model.live='servicePrice'>
                            <h3 class="text-lg font-semibold text-center mb-4">Paket 6 Bulan</h3>
                            <div>
                ...
                <x-input-error :messages="$errors->get('servicePrice')" class="mt-2 text-red-300" />
        ...
        <div class="w-full lg:w-[60%] px-4">
            <div class="p-4 md:p-6 bg-white shadow-xl shadow-neutral-200 rounded-2xl">
                ...
                <div class="mt-6" wire:ignore>
                    ...
                    <div id="{{ $qldeskripsi }}">{!! $description !!}</div>
                ...
                <div class="mt-6 grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <x-input-label class="text-neutral-500">Kota*</x-input-label>
                                ...
                                <div class="">
                                    ...
                                    @if ($image['preview'])
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="text-pretty max-w-40 truncate">
                                                <p>{{ $image['name'] }}</p>
